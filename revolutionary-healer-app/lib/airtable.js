...
// =============================================================================
// CHAT-DRIVEN SHIFT CREATION (Aug 20, Rachael's "Shift + Activation
// Follow-Through" spec)
// =============================================================================
// Unlike createGapMethodShift above (which only ever fires from a completed
// 3 Step GAP Method diagnostic), a Shift can now also be created directly
// from an ordinary Revolutionary Healer AI conversation once a Gap becomes
// clear organically and the member explicitly confirms she wants it saved --
// see app/api/chat/route.ts's [[SAVE_SHIFT: ...]] marker detection and
// lib/prompts.js's SHIFT + ACTIVATION FOLLOW-THROUGH section for the rules
// governing when the AI is allowed to actually emit that marker. SHIFT CARD
// CREATION RULE still applies here: one Shift per distinct Gap/area, never
// one per conversation -- route.ts checks the member's existing Shifts
// (getShiftsByEmail above) and calls updateShiftFields on a matching one
// instead of this function whenever the discovery continues a contradiction
// already being worked, rather than creating a duplicate card.
export async function createShiftFromChat({
    ...
    // Link the originating chat back to this Shift (ChatSessions.linked_shift_id,
// already part of the schema -- see SPEC.md's Chat History + Memory doc) so
// the Shift's origin conversation can always be found again later.
if (chatId) {
    try {
...
// lib/shifts.js's EMBODIED STATUS rules for when each should be set. Also
// reused (Aug 20) by the SHIFT + ACTIVATION FOLLOW-THROUGH [[UPDATE_SHIFT]]
// path above to refresh gap_explanation / what_we_noticed /
// recommended_activation / current_frequency on an existing chat-continued
// Shift -- never touches `undercurrent`, which stays reserved for the
// formal "Go Deeper Into This Gap" evidence/consent flow.
